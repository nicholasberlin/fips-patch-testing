...
// fetchTLS13-KDF fetches the TLS13-KDF algorithm.
// It is safe to call this function concurrently.
// The returned EVP_KDF_PTR shouldn't be freed.
...
func newHKDFCtx3(md C.GO_EVP_MD_PTR, mode C.int, secret, salt, pseudorandomKey, info []byte) (_ C.GO_EVP_KDF_CTX_PTR, err error) {
	...
	if useTLS13KDF {
		if (mode == C.GO_EVP_KDF_HKDF_MODE_EXTRACT_ONLY) {
			...
			bld.addOctetString(C.CString("prefix"), []byte("tls13 "))
			bld.addOctetString(C.CString("label"), label)
			bld.addOctetString(C.CString("data"), context)
			if len(pseudorandomKey) > 0 {
